 ...
 *
 * Planning and committing are deliberately separate calls (see `planIcsImport`)
 * so the confirm dialog and the import itself cannot disagree about what is
 * about to happen.
 */
import { freeEntryKey, parseToStoreItems } from '@/model'
 ...
 *
 * The whole plan is assembled before anything leaves this function, so a file
 * whose last event fails to parse throws having changed nothing, rather than
...
export function planIcsImport(vaultId: string, text: string): IcsImportPlan | null {
  ...
  let added = 0
  let updated = 0
  for (const event of synthesis.entries) {
