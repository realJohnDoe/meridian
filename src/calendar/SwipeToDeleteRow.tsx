...
interface Props {
  ...
  /** View-only vaults (an iCal subscription) have no source to write back to — see hooks/useEntryAccess. */
  disabled?: boolean
  children: ReactNode
}

/**
 * The swipe-to-delete gesture, shared by every agenda row that offers it
 * (AgendaRow, AgendaOverdueGroupRow). Deletes whatever `occ` is passed —
 * callers decide what that occurrence represents; for a grouped overdue row
 * that's the oldest occurrence standing in for the series, and deleting it
 * has exactly the same effect the row's checkbox already has: the count
 * drops by one and the next-oldest becomes the representative.
 */
export default function SwipeToDeleteRow({ occ, onSwipeDelete, disabled, children }: Props) {
  ...
  }, [disabled]) // listeners are stable; callback accessed via ref

  return (
    // overflow-hidden clips the delete panel to the row's rounded corners and
