...
const EVENT_LEFT = GUTTER - 6  // event blocks start a hair left of the grid edge
...
function EventBlock({ o, dh, colIndex, totalCols, onOpen }: EventBlockProps) {
  ...
  const left  = `calc(${EVENT_LEFT}px + ${colIndex} * (100% - ${EVENT_LEFT}px) / ${totalCols})`
  const width = `calc((100% - ${GUTTER}px) / ${totalCols} - 3px)`
