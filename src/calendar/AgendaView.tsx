...
export default function AgendaView({ onOpen }: Props) {
  ...
  //
  // scrollToIndex, not a direct seeded scrollTop write: a version of this that
  // wrote scrollTop = offsetOfRow(...) directly (summing against either
  // calendarView.getState().agendaScrollMeasurements or
  // virtualizer.takeSnapshot()) shipped briefly and broke both the Today
  // button and the cold-start landing. Two distinct, compounding bugs, not
  // one:
  //   1. agendaScrollMeasurements is written only on unmount (see
  //      useSaveAgendaScroll below) — during a live session it is stale or
  //      empty, so the summed offset was reliably wrong. Repeated presses
  //      each summed against the same stale snapshot instead of converging,
  //      which read as drift that never settled.
  //   2. Even switched to virtualizer.takeSnapshot() (the virtualizer's own
  //      live cache — correct for *that* problem), a single bounded
  //      corrective rAF pass turned out not to be enough: if `rows` changes
  //      again before that frame (a background vault sync landing more
  //      content, which routinely happens within the first frame or two
  //      after a cold mount), this effect's own cleanup cancels the pending
  //      correction, and by then agendaScrollTarget has already cleared
  //      (markAgendaScrolled below), so nothing re-arms it.
  //      useAnchoredAgendaScroll's own "hold position" mechanism then takes
  //      over — from the *uncorrected* estimate this effect wrote, not the
  //      true target — landing on neither.
  // scrollToIndex sidesteps both: it drives virtual-core's own
  // reconcileScroll, which re-resolves the target from the index on every
  // frame until it is stable, so a `rows` change mid-reconciliation is
  // exactly the case it's built to survive. See
  // plans/agenda-quicknav-swipe.md's "PR 2 was reverted" note for the
  // measurement that made this not worth re-attempting: the direct-write
  // approach's own performance win, re-measured cleanly, was statistically
  // indistinguishable from this scrollToIndex baseline.
  useLayoutEffect(() => {
