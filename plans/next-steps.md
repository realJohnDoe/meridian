...
1. Entry-editing concern is split across 6 files with inverted dependencies
   Category: architecture srp
   Impact: 8
   Evidence: mutations.ts:12 — import type { EntryState, ItemType } from './components/EntryEditor'; the editor concern spans EntryEditor.tsx, EditorShell.tsx, useEntryEditor.ts, EntryOverlay.tsx, DialogStack.tsx, and mutations.ts, with the hook importing from mutations, mutations importing types from the component, and EditorShell re-importing the hook's return type.
   Problem: The domain mutation layer depends upward on a UI component for its canonical form-state type, so editor state has no owner — any change to entry fields touches six files in three layers.
   Fix: Move EntryState/ItemType into a src/editor/ (or types.ts) module that owns editor state, and co-locate the shell/hook/dialog-stack/overlay files under it so the dependency arrow points UI → domain only.

2. vault.ts is a god module that bypasses its own store abstraction
   Category: architecture srp
   Impact: 7
   Evidence: vault.ts:21-22 imports both storeBridge and useStore directly; raw useStore.setState calls at vault.ts:33, 180, 257-258 (UI flash animation timing inside the sync engine: setTimeout(() => useStore.setState({ syncFlash: false }), 800)); the file also owns conflict resolution, cache hydration, vault registry CRUD, and permission flows in 469 lines.
   Problem: The storage layer directly drives UI presentation state (syncFlash, scrollToTodayOnce) and duplicates two access paths to the same store, so storage and UI-state domains can't evolve independently.
   Fix: Split into vaultRegistry.ts (add/remove/restore) and sync.ts (reconcile/collision/dirty-tracking), route all store writes through storeBridge, and move flash/scroll UI flags into UI-side subscriptions to sync events.

3. The core expansion engine is effectively untyped
   Category: types architecture
   Impact: 7
   Evidence: expansion.ts:122-128 — ExpandNode has [key: string]: unknown with date?: unknown; expandNode returns Record<string, unknown>[] and expandRange re-casts (occ.jsTime as Date, String(occ.date ?? '') at expansion.ts:490-514); 11 as-casts in this file, 15 in storeOps.ts.
   Problem: The most algorithmically dense domain logic in the app (recurrence, overrides, exclusions) has zero compile-time field checking, so typos in field names or shape drift between StoreItem and the duck-typed engine surface only as wrong calendar output.
   Fix: Give expandNode a typed input/output (a narrow ExpandNode with concrete date: string, metadata: OccurrenceMetadata) and do the YAML coercion once at the storeItems.ts boundary instead of inside the engine.

4. Two parallel styling systems with a cryptic class vocabulary
...
5. TODAY is frozen at app startup in an installable PWA
   Category: ux architecture
   Impact: 6
   Evidence: constants.ts:4-6 — "Computed once at startup"; consumed by agenda grouping (AgendaView.tsx:23-31), applyScope (mutations.ts:42), fileOccurrenceMap window (presentation.ts:98), nav (\_app.tsx:75-87) — while occState uses live new Date() (presentation.ts:208-214).
   Problem: This is a PWA (vite-plugin-pwa, standalone display) that users keep open; after midnight the agenda's "Today" header, sort dimming, and date pickers disagree with the live clock used by occState, showing yesterday as today.
   Fix: Replace the constant with a useToday() hook (or store field refreshed by a minute-interval/midnight timer) and keep one time source for both grouping and state classification.

6. Flat components/ directory spanning five unrelated domains
   Category: layout
   Impact: 5
   Evidence: src/components/ holds 28 files mixing calendar views (AgendaView, DayView, MonthView, DaySection), the entry editor and its 7 dialogs (DatePickerDialog, TimePickerDialog, DurationDialog, RepeatDialog, PriorityDrawer, DeleteDialog, SeriesDeleteDialog), vault management (AddVaultDialog), and search (FilterOverlay, FileResultsList) in one level.
   Problem: Files that always change together (editor + its dialogs + DialogStack) live alongside files that never interact, so co-change sets aren't discoverable and the route layer can't own its views.
   Fix: Group into components/calendar/, components/editor/, components/vaults/, components/search/ (keeping ui/ as is), which also makes finding 1's editor-module extraction natural.

7. Every view re-runs full expansion; fileOccurrenceMap expands ±3 years
...
8. Three sources of truth for a task's done state, including prop mutation
   Category: architecture dry
   Impact: 5
   Evidence: mutations.ts:112 — o.metadata.done = !o.metadata.done // optimistic UI mutates a derived expanded occurrence in place; OccurrenceRow.tsx:20,29 keeps a local isDone mirror re-synced via useEffect; the store's toggleDone result is the third copy.
   Problem: Mutating an object owned by the (supposedly immutable) zustand-derived expansion output is undefined behavior under React, and the local mirror exists only to paper over it — done-state bugs (stale checkbox, flash of wrong color) have no single place to fix.
   Fix: Make toggleDone's setData the only write, delete the in-place mutation and the isDone local state, and rely on the store update for rendering (it's synchronous).

9. Editor field shape redefined three times with manual remapping
   Category: dry naming
   Impact: 5
...
10. Recurrence-interval logic duplicated between dialog and engine
    Category: dry srp
    Impact: 4
...
11. Persistence is fire-and-forget; UI confirms saves before they happen
...
12. GitHub PAT stored in plaintext IndexedDB
    Category: security
    Impact: 4
...
13. Vault registry update ritual duplicated four times
    Category: dry
    Impact: 3
...
14. Identical dedupe/sort block copy-pasted in the expansion API
    Category: dry
    Impact: 3
...
15. Notification system implemented twice end-to-end
    Category: dry ux
    Impact: 3
...
16. Date primitives re-implemented in three modules
    Category: dry
    Impact: 3
...
17. MonthView filters all month occurrences once per cell
    Category: performance
    Impact: 3
...
18. Storage-layer names still describe the local-disk era
    Category: naming
    Impact: 2
...
19. Dead code at module boundaries
    Category: dead-code
    Impact: 2
...
20. Unbounded module-level caches in the model layer
