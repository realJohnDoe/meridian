...
2. onOpen = navigate(entryRoute(...)) duplicated across route files
   Category: dry
   Impact: 3
...
3. Stateful toast scheduler lives as module globals inside mutations.ts
   Category: srp architecture
   Impact: 4
...
4. Editor handler prop-drilling through three layers
...
5. Dead no-op spread + unused binding in updateRoot
   Category: dead-code
   Impact: 2
...
6. initApp() is an empty no-op still wired into the root
   Category: dead-code
   Impact: 1
...
7. GitHub backend only sees the repo root and keys files by name, not path
...
8. No global loading/error UI while the vault restores
   Category: ux
   Impact: 3
...
9. Occurrence visual state is stringly-typed and decoded by ad-hoc maps
...
10. Navigation hard-bound to window.history.back()
...
11. notify() builds error strings by hand and re-implements an auto-dismiss timer
...
12. EntryEditor repeats the metadata-chip button five times
    Category: dry
    Impact: 2
