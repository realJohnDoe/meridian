...
3. Dead no-op spread + unused binding in updateRoot
   Category: dead-code
   Impact: 2
...
4. initApp() is an empty no-op still wired into the root
   Category: dead-code
   Impact: 1
...
5. GitHub backend only sees the repo root and keys files by name, not path
...
6. No global loading/error UI while the vault restores
   Category: ux
   Impact: 3
...
7. Occurrence visual state is stringly-typed and decoded by ad-hoc maps
...
8. Navigation hard-bound to window.history.back()
...
9. notify() builds error strings by hand and re-implements an auto-dismiss timer
...
10. EntryEditor repeats the metadata-chip button five times
    Category: dry
    Impact: 2
