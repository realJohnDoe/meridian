...
1. The core expansion engine is effectively untyped
...
2. Two parallel styling systems with a cryptic class vocabulary
...
3. Every view re-runs full expansion; fileOccurrenceMap expands ±3 years
...
4. Three sources of truth for a task's done state, including prop mutation
...
5. Editor field shape redefined three times with manual remapping
   Category: dry naming
   Impact: 5
...
6. Recurrence-interval logic duplicated between dialog and engine
   Category: dry srp
   Impact: 4
...
7. Persistence is fire-and-forget; UI confirms saves before they happen
...
8. GitHub PAT stored in plaintext IndexedDB
   Category: security
   Impact: 4
...
9. Vault registry update ritual duplicated four times
   Category: dry
   Impact: 3
...
10. Identical dedupe/sort block copy-pasted in the expansion API
    Category: dry
    Impact: 3
...
11. Notification system implemented twice end-to-end
    Category: dry ux
    Impact: 3
...
12. Date primitives re-implemented in three modules
    Category: dry
    Impact: 3
...
13. MonthView filters all month occurrences once per cell
    Category: performance
    Impact: 3
...
14. Storage-layer names still describe the local-disk era
    Category: naming
    Impact: 2
...
15. Dead code at module boundaries
    Category: dead-code
    Impact: 2
...
16. Unbounded module-level caches in the model layer
