...
1. expansion.ts is an 855-line god-module mixing date utils, duration parsing, tree expansion, and metadata join — with a wide dead/internal-only export surface
...
2. Dual styling system: ~84 hand-written component CSS rules alongside inline Tailwind + shadcn
   Category: styling
   Impact: 5
...
3. AppLayout (\_app.tsx) is a god-component owning ~7 unrelated concerns
...
4. exampleRef literal hand-rolled 4× in vault.ts
   Category: dry
   Impact: 3
...
5. onOpen = navigate(entryRoute(...)) duplicated across route files
   Category: dry
   Impact: 3
...
6. Stateful toast scheduler lives as module globals inside mutations.ts
   Category: srp architecture
   Impact: 4
...
7. Editor handler prop-drilling through three layers
...
8. Dead no-op spread + unused binding in updateRoot
   Category: dead-code
   Impact: 2
...
9. initApp() is an empty no-op still wired into the root
   Category: dead-code
   Impact: 1
...
10. GitHub backend only sees the repo root and keys files by name, not path
...
11. GitHub PAT persisted in plaintext IndexedDB
    Category: security
    Impact: 3
...
12. No global loading/error UI while the vault restores
    Category: ux
    Impact: 3
...
13. Occurrence visual state is stringly-typed and decoded by ad-hoc maps
...
14. Navigation hard-bound to window.history.back()
...
15. notify() builds error strings by hand and re-implements an auto-dismiss timer
...
16. EntryEditor repeats the metadata-chip button five times
    Category: dry
    Impact: 2
