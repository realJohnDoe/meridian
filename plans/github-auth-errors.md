...
PR 3 (`needsAttention` replaces `needsReconnect`, state only) is implemented —
see `store.ts`'s `VaultAttention`/`AttentionKind` and their writers in
`storage/vaultRegistry.ts` and `storage/sync.ts`.**
