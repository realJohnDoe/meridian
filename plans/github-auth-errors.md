...
`src/storage/failureKind.ts`.**

**PR 3 (`needsAttention` replaces `needsReconnect`, state only) is
implemented — see `store.ts`'s `VaultAttention`/`AttentionKind` and their
writers in `storage/vaultRegistry.ts` and `storage/sync.ts`.**
...
**PR 2 alone fixes Report B** and retires the misleading message. **PR 7
