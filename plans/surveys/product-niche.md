...
- **Make positioning measurable rather than a matter of taste:**
  ...
  - **Term census.** Count the words each surface uses for the same concept across README, in-app copy, tutorial entries, and blog — deriving the counts yourself, and stating the method (whole-word? case-insensitive? does the `src/` count include identifiers as well as user-visible strings?), because a raw `grep` over `src/` counts code symbols that no reader ever sees. The method that worked on the last run, if you want a default: whole-word and case-insensitive, and for `src/` count only quoted string literals and JSX text in non-test `.tsx` files (`grep -ohE '("[^"]*"|'\''[^'\'']*'\''|>[^<>{}]+<)'`) — for "vault" that gave 33 user-visible strings against 1,749 raw matches, so the choice of method changes the finding by two orders of magnitude. Report both numbers. **These two are dated examples, not targets** — the same method returned 37 and 1,996 on 2026-09-14. Re-derive them; the ratio is the finding, the figures are not, and quoting them from here instead of measuring is the error this bullet exists to prevent. Vocabulary that fractures — "vault" dominating the UI source while the README barely uses it, the tour saying "a folder you own", getting-started saying "repo" — makes a product harder to recognise, because the reader can't tell whether they're being sold one thing or three.
  - **Emphasis map.** Compare the space each surface gives a capability against how central that capability is to the declared niche's job. A capability argued at length that the niche doesn't need, and a niche-defining capability mentioned once, are both findings. **State a method here too**, for the same reason the term census does — "feels over-long" is not evidence. What worked on the 2026-09-14 run: split `README.md` on `\n(?=## )` and again on `\n(?=### )`, count `.split()` words per section and report each as a share of the whole; then put those shares beside non-test line counts per `src/` directory (`find <dir> \( -name '*.ts' -o -name '*.tsx' \) ! -name '*.test.*' | xargs wc -l`). That pairing is what makes "a 1,948-line capability and a 313-line one get the same bullet" a measurement rather than an impression.
...
- **Derive the revealed niche from the product, not the docs:** the feature surface (which capabilities are deep — recurrence, wikilinks, participants, search — and which are thin; line counts per `src/` directory are a fast first cut), the storage options and what each demands of a user (`src/storage/`, plus the trade-offs already recorded in [storage-backend.md](../reports/storage-backend.md)), platform and browser constraints, the entry-format surface area in `src/model/fieldRegistry.ts` (`INLINE_FIELDS`, `STRUCTURAL_KEYS`) and the rest of `src/model/`, and what the default first-run path optimises for.
...
- **If no preview tooling is available in the session at all** — check before planning around it — the fallback is not the first resort. The repo already carries `@playwright/test` as a devDependency (it is what `e2e/` and `scripts/perf/stress.mjs` drive — `playwright-core` was dropped as a direct dependency when `scripts/layout-smoke.mjs` became `e2e/layout-smoke.spec.ts` in PR #1014, so the recipe this paragraph gave until 2026-09-13 pointed at a package that is no longer installed), so a session with Bash and a Chromium binary can still take real screenshots: start the server yourself (`pnpm exec vite --port <unique> --strictPort`), then drive it from a scratch script — importing `@playwright/test` by absolute path if the script lives outside the repo — at 390×844 and 1440×900, setting `localStorage['meridian_theme']` in an init script to reach each palette. That path was taken on the last run and category 3 came back fully assessed. Only if that is also unavailable: do not silently skip the visual work or quietly substitute a guess for it. Say so in the coverage statement, fall back to assessing visual identity from `src/index.css` tokens, the dependency set, and layout source, mark category 3 **partially assessed**, and flag any finding that rests on the un-taken screenshots as needing confirmation before it is acted on. An impression invented from tokens is worse than an admitted gap.
...
- **Bounded competitor verification only**, per the scope note: confirm the comparison table's specific factual claims, nothing further. **Expect the vendors' own docs to be unreachable** — on 2026-09-14 both `tasknotes.dev` and `support.google.com` returned `EGRESS_BLOCKED` from this environment's proxy, which is the first thing a run hits and the easiest place to give up and mark a claim unverified. `WebSearch` and the vendors' secondary hosts are not blocked, and were enough: `github.com/callumalpass/tasknotes` settled TaskNotes' licence and its per-instance recurrence handling, and `developers.google.com/calendar` settled Google Calendar's. A claim you could not reach a source for goes in the coverage statement's verification table as **could not verify** with the blocked host named — never as "verified" from memory, and never silently dropped.
...
- **Who it costs us** — the person who bounces, misjudges, or churns because of it, and roughly what share of arrivals are that person. **"Unknown" is a permitted and often correct answer**: this field must not become the back door through which invented demand claims re-enter the report. Where the share is knowable from an artifact (which surface carries it, how far down the page it sits, whether it is on the default path), say so and how you know; where it isn't, write "unknown" and add it to the bet list in section 5 instead of estimating. **A run in which every finding's share reads "unknown" is a correct run, not a lazy one** — until this product has a feedback path, that is the honest answer every time, and the 2026-09-14 run returned exactly that. Say so once in the coverage statement, with what the field *was* able to establish from artifacts (which surface carries the claim, how far down the page, whether it sits on the default path), rather than reaching for a number to avoid an empty-looking column
