...
- **"Great mobile UX" is the first row of the comparison table**, i.e. the claimed lead differentiator, and `CLAUDE.md` describes a mobile-focused PWA. Verify that the product's investment matches that claim, and that the surfaces sell it as _the_ lead rather than as one of ten equal-weight rows.
- **The declared audience and the front-door setup path may select different people.** The pitch is calm and file-based; the recommended way in is a GitHub repository (with a hand-made fine-grained token as fallback), and local folders require a Chromium browser. Determine which niche the _setup path_ actually selects, and whether it's the one the copy targets.
