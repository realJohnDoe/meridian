...
route (#1, and worth 53% of the initial bundle); strictness flags applied to the
...
| 2 | **#1** | Root route's barrel import defeats the editor's own `lazy()` — 53% of the initial bundle | `performance` `architecture` `layout` | 8 | 5 | **Sonnet 5** | 20.0 |
| 3 | **#4** | Five coverage floors have drifted 10–26 points below measured | `testing` `toolchain` | 4 | 5 | **Haiku 4.5** | 20.0 |
| 4 | **#6** | Primary vault-onboarding and entry-view screens have ~0% coverage | `testing` | 5 | 6 | **Sonnet 5** | 15.0 |
| 5 | **#3** | 20 evaluated `strict-type-checked` rules were written up but never enabled | `toolchain` `types` | 4 | 7 | **Sonnet 5** | 14.0 |
| 6 | **#9** | `startGitHubSignIn` is the one vault action that skips the catch-and-notify convention | `error-handling` | 3 | 4 | **Sonnet 5** | 6.0 |
| 7 | **#5** | `/ical` Worker endpoint is an unauthenticated open fetch proxy | `security` | 5 | 2 | **Opus 5** | 3.3 |
| 8 | **#7** | `components.json` `utils` alias points at a module that doesn't exist | `toolchain` `library-fit` | 2 | 1 | **Haiku 4.5** | 2.0 |
| 9 | **#8** | `CLAUDE.md`'s "Route shells" warning describes a gap that is now closed | `toolchain` | 2 | 1 | **Haiku 4.5** | 2.0 |

**The formula has inverted the order here, in both directions — read this
before sorting by the Score column.**

- **A reader sorting by raw impact should start at #1, not #2.** #1 is the only
  impact-8 finding in this report and the only one a user can feel: it is 53% of
  what every visitor downloads before the app paints. #2 ranks above it purely
  because its fix is a five-line config edit that a dry-run proved costs zero
  code changes. Both are worth doing; #1 is the one that matters.
- **#5 (security, impact 5) falls to 7th purely on the effort divisor**, because
...
existing ones, so doing #4 last folds both into one coherent pass. #2, #3, #5, #7
