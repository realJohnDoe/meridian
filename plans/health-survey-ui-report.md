...
### 1. `@eslint-react` is configured as a hand-picked subset of a preset that ships 5× more

- **Category** `toolchain`
- **Impact** 5
- **Breadth** 1 config file, gating 8 real violation sites across 7 source files (from the dry-run
  below)
- **Recommended model** **Sonnet 5.** The hazard is that turning on the full preset in one go mixes
  ...
  only the non-overlapping ones — chiefly `purity`. Without that instruction named, it needs Opus 5
  to work out the overlap.
- **Evidence** `eslint.config.js` enables exactly 13 rules by hand:
  ...
  @eslint-react/purity | src/editor/dialogs/RepeatDialog.tsx:257 | Do not call 'new Date()' during render. Components and hooks must be pure.
  @eslint-react/no-unnecessary-use-prefix | src/calendar/useAgendaScrollRestore.ts:30
  @eslint-react/naming-convention-context-name | src/components/ui/responsive-modal.tsx:8
  ```
...
- **Fix** Spread `reactPlugin.configs['recommended-type-checked'].rules` alongside the existing
  `reactHooksRules`, then explicitly `'off'` the four rules that duplicate `react-hooks` equivalents,
  keeping the existing per-rule comments for anything downgraded.
...
### 7. `RepeatDialog` hand-rolls a second date picker; `DatePickerDialog`'s `forceDialog` escape hatch was built for this and is dead

- **Category** `dry` `component-architecture` `library-fit` `dead-code`
- **Impact** 5
- **Breadth** 2 files (`editor/dialogs/RepeatDialog.tsx`, `editor/dialogs/DatePickerDialog.tsx` —
  confirmed by `grep -rn "forceDialog" src`, which shows the prop declared, threaded, and never passed
  by any caller)
- **Recommended model** **Sonnet 5.** The hazard is a behavioral difference between the two pickers
  that is easy to lose in the swap: the inline one commits on `onSelect` and closes immediately
  (`setEndVal(fmtISO(date)); setEndCalOpen(false)`), whereas `DatePickerDialog` stages into local
  `selected` state and commits only on the Set button. Substituting it without mapping `onConfirm` →
  `setEndVal` and `onRemove` → clearing `endVal` produces a picker that opens, looks identical, and
  silently discards the chosen date. With that mapping named in the task, this is a well-bounded swap
  and `RepeatDialog.test.tsx` plus the coverage floor on that file guard it.
- **Evidence** `DatePickerDialog.tsx:23` declares the prop for precisely this case:
  ...
  and `RepeatDialog.tsx:466` reaches the same conclusion — then rebuilds the picker from scratch
  anyway:
...
- **Problem** The app's one date picker exists twice with divergent commit semantics and layout, so
  ...
  weight that reads as covered.
- **Fix** Replace `RepeatDialog`'s nested calendar block with `<DatePickerDialog forceDialog … />`,
  mapping `onConfirm`/`onRemove` onto `setEndVal`.
...
### 9. Global `Cmd/Ctrl+B` toggles the sidebar — stealing bold inside the Markdown editor

- **Category** `ux` `library-fit`
- **Impact** 5
...
- **Recommended model** **Haiku 4.5.** The fix is a guarded early-return in one `keydown` handler and
  ...
  event target is inside `.cm-editor`, or an `input`/`textarea`/`[contenteditable]`) and this is a
  two-line change.
- **Evidence** `components/ui/sidebar.tsx:27` and the handler at line 94, inherited verbatim from the
  shadcn sidebar template:
...
- **Problem** In an app whose premise is Markdown editing, the universal "bold" chord is silently
  ...
  the writer.
- **Fix** Guard the handler to ignore events originating inside `.cm-editor`, `input`, `textarea`, or
  `[contenteditable]` — or drop the shortcut entirely, since the app has no other keyboard shortcuts
  and the sidebar has a visible trigger.

---

### 10. Duplicated UI constants and dead `data-tour` markup
...
- **Recommended model** **Haiku 4.5.** All four items are mechanical and fail loudly — a bad constant
  ...
  One hazard to name so it isn't tripped over: `src/search/FileResultsList.test.tsx:141` asserts
  `container.querySelectorAll('[data-tour="entry-card"]')`, and `CLAUDE.md` documents
  `[data-tour="entry-card"]` as the preview-navigation selector — so removing the attribute requires
  updating that test and that doc line in the same change, not just the JSX.
- **Evidence** The same three-entry map appears in both `editor/EntryEditor.tsx:48` and
  `editor/dialogs/PriorityDrawer.tsx:15`, character for character:
...
- **Fix** Move `PRIORITY_CLASS` next to the other priority tokens in
  `components/ui/occurrence-variants.ts`, delete the four `data-tour` attributes (updating
  `FileResultsList.test.tsx` and the CLAUDE.md preview note to a stable selector in the same change),
  collapse `TagChip`'s two branches onto one shared body, and factor `BacklogView`/`NotesView` into
  one component parameterised by kind, icon, and copy.
