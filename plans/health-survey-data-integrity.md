...
- **No property-based testing is installed** (`fast-check` is absent from `package.json`). Round-trip and hoisting invariants are exactly the shape property testing is built for. Evaluate whether adopting it is worth it here — with a dry-run generator over real vault shapes, reporting what it actually finds — and say honestly if example-based fixtures are sufficient.
  - **REFUTED as a priority — don't install it yet.** All eight findings came from hand-authored examples, and **none** would have been caught by a generator over well-formed store states: #2, #3 and #5 are all cases where the input space such a generator samples (valid `StoreItem[]`) already excludes the shape that breaks. A generator would ratchet the hoisting algorithm — the part that is already correct. **If adopted later**, generate *source YAML* rather than store states and assert set-containment of key/value pairs — the assertion `collectKeyValues` in `__tests__/helpers.ts` already implements. Fix #2/#3/#5 first.

- **`planReconcile` is pure and separate from `reconcileWithBackend`.** That split makes the reconcile decision table directly unit-testable; check whether the tests actually enumerate its cases (local-dirty + remote-changed, delete-vs-edit, tombstone races) or only the happy paths.
  - **REFUTED — this is the best-tested code in the survey.** `reconcile.test.ts` enumerates 16 cases across all four branches: never-seen, version-drift, dirty + remote-changed, tombstone + still-listed, `skipPaths` for just-pushed / just-created / just-deleted, vanished-file drops for clean / dirty / tombstone, and four grace-window cases including the exact boundary. The untested neighbour is **`resolveCollision`** — the function `pushDirty` hands a conflict to — which is where findings #1 (a failed copy write destroys the local edit) and #4 (a conflict copy per sync tick, forever) both live.
