...
- **Failure mode:** **silent.** The user sees their new entry created normally. The
  destroyed entry simply stops existing; wikilinks pointing at it now resolve to the new
  content.
- **Impact:** **9** — silent, unrecoverable loss of an entire existing entry on a common
  path (creating a note).

**Repro.** Starting file `buy-groceries.md`, verbatim:
...
Operation: create a new entry titled **`Buy groceries!`** with body `totally different
note` — i.e. `applyEdit(snapshot, null, 'add', fields)`, which is what
`saveNode(null, …)` calls (`src/editor/save.ts:106`).

Observed content of `buy-groceries.md` afterwards:
...
Expected: a second file, or a visible "an entry with that name already exists" error.
The tags, the items checklist, `done`, `date` and the body are all gone with no artifact.

`titleToSlug` collides freely — every pair below maps to one slug:

```
"Buy groceries"   / "Buy groceries!"    -> buy-groceries
"Meeting w/ Bob"  / "Meeting w Bob"     -> meeting-w-bob
"1:1 with Alex"   / "1 1 with Alex"     -> 1-1-with-alex
"Café"            / "Cafe"              -> cafe
"Q3 review"       / "Q3-review"         -> q3-review
```

plus any two titles agreeing in their first 60 slug characters (the `.slice(0, 60)`).
...
- **Breadth:** one function, `applyNew` (`src/model/storeOps.ts:213–224`), reachable from
  every new-entry path (`saveNode` with `item === null`; `grep -rn "commitNext(\|commitDelete("`
  finds 10 commit sites, of which the editor's save is the one that can carry a null
  item). The exposed set is any pair of vault entries whose titles agree after
  lowercase + de-accent + non-alphanumeric collapse + 60-char truncation.
- **Recommended model:** **Opus 5** — **Sonnet 5** if the task explicitly states the two
  constraints below. The hazard: the guard is not an accident, it is load-bearing. Its
  own doc comment explains it exists because "a 'new entry' commit can run more than once
...
- **Problem:** a new entry silently adopts and overwrites an unrelated existing file
  whenever their titles slugify the same, costing the user that file's entire content.
- **Fix:** carry the draft's identity into `applyEdit` so `applyNew` can tell "this draft
  saving again" from "a different entry on a taken slug", and disambiguate the latter
  (suffix the slug, or refuse with a visible error). After the fix the repro must leave
...
### #3 — The local FS backend ignores the CAS contract on create and on delete

- **Invariant violated:** 4 (no lost update). Local-folder vaults only, whenever a second
  writer touches the folder — another device via Dropbox/iCloud/Syncthing, or the user in
  Obsidian/VS Code. Both windows are wide: a tombstone persists in Dexie across reloads
...
- **Impact:** **8** — silent, unrecoverable overwrite/destruction of content the app
  never saw, on a supported backend, with the intended safety net inert.
...
```
diskWrite(dh, 'meeting-notes.md', 'APP CONTENT', undefined)
→ returns "1785154682027:11"
→ file on disk now: "APP CONTENT"
```
...
```
diskDelete(dh, 'a.md')          // LocalBackend.delete drops _expectedVersion
→ file still present? false
```

Observed: the hand edit is destroyed, no `ConflictError`, no warning. Expected: the
tombstone-conflict branch in `pushDirty` fires and keeps the remote version.
...
- **Breadth:** 2 methods in 1 of 3 backends (`grep -rln "implements StorageBackend" src`
  → `localBackend.ts`, `githubBackend.ts`, `exampleBackend.ts`; the example one is
...
- **Recommended model:** **Sonnet 5 if the task spells out the two preconditions to
  ...
  a permanent retry loop (that comment is already in the code and explains why). Both
  failures are loud-ish (a test breaks), which is what keeps this off the top tier.
- **Evidence:** `src/storage/localBackend.ts:21` — the parameter is named and discarded:

  ```ts
    delete(path: string, _expectedVersion?: string): Promise<void>    { return diskDelete(this._handle, path) }
  ```

  and `src/storage/fs.ts:148`, where the CAS block only runs when a version was supplied:
  ...
  against the contract in `src/storage/backend.ts:40`:
...
- **Problem:** on a local-folder vault a push can overwrite or delete content the app
  never read, so an edit made in another editor or synced from another device disappears
  with no conflict copy and no warning.
- **Fix:** enforce absence-as-precondition in `diskWrite` when `expectedVersion` is
  undefined, and pass `expectedVersion` through `LocalBackend.delete` into a stat-check in
  `diskDelete` (keeping `NotFoundError` → success). After the fix both repro cases throw
...
### #4 — Delete-undo restores a whole-store snapshot, reverting unrelated edits and desyncing store from cache

- **Invariant violated:** 2 (edit locality), 5 (cache coherence), 6 (durability). Any
  swipe-delete followed by any other edit inside the 4-second undo window
  (`TOAST_MS = 4000`).
- **Category:** `recoverability` `cache-coherence` `edit-locality`
- **Failure mode:** **silent.** After undo the UI shows the reverted (wrong) state while
  the cache and backend hold the correct one. A reload flips the UI back — so the user
  sees a value change by itself. If instead they edit that entry from the stale view, the
  stale value is written back and the real edit is destroyed.
- **Impact:** **7** — silent divergence on a common gesture pair, escalating to real loss
  on the next edit of the affected file.

**Repro.** Two entries, `a.md` (a dated occurrence) and `b.md` (a task with `done: false`).

1. Swipe-delete the occurrence in `a.md`. → store: `slugs: ['b']`, undo toast up.
2. Within 4 s, tick the task in `b.md` done. → store: `b done = true`; persistence
   records `writes: ['b']` (the cache now holds `done: true`).
3. Press **Undo** on the delete toast.

Observed:

```
AFTER UNDO — slugs: [ 'a', 'b' ]
AFTER UNDO — b done = false        ← the unrelated tick was reverted
AFTER UNDO — writes: [ 'b' ]       ← and no write was issued to correct the cache
```

Expected: `a` restored, `b done = true` untouched.
...
- **Breadth:** one file, `src/occurrenceActions.ts`, two undo closures (lines 100 and
  108–110). Reachable from every swipe-delete row in the agenda, day and month views.
- **Recommended model:** **Sonnet 5.** The hazard that sets the tier: narrowing the
...
### #6 — Month-end overflow moves monthly occurrences to the wrong day and skips whole months

- **Invariant violated:** 8 (temporal correctness). `bymonthday` values above 28, and
  yearly series anchored on Feb 29.
- **Category:** `temporal`
- **Failure mode:** **silent.** A rent reminder simply appears on May 1 and not in
  February; nothing indicates an error.
- **Impact:** **6** — an occurrence silently moves days and another silently vanishes;
  ticking the misplaced one writes an override on the wrong date into the file.
...
Operation: `expandRange(items, roots, new Date(2026, 0, 1), new Date(2026, 5, 30))`.

Observed: `2026-01-31, 2026-03-31, 2026-05-01, 2026-05-31`

Expected: `2026-01-31, 2026-03-31, 2026-05-31` (and, per whatever policy the project
picks, something or nothing for the 30- and 28-day months — but never **May 1**, which is
not the 31st of anything).

Two independent overflows produce this. `new Date(2026, 3, 31)` for April rolls forward to
May 1. Separately, the cursor `nextBase` does `n.setMonth(n.getMonth() + 1)` on Jan 31,
giving "Feb 31" → **Mar 3**, so February's period is never visited at all.

The same defect in the yearly branch:

```markdown
---
title: Bday
date: "2024-02-29"
repeat: { type: schedule, freq: yearly }
---
```

Observed over 2024–2028: `2024-02-29, 2025-03-01, 2026-03-01, 2027-03-01, 2028-02-29` —
a Feb-29 birthday lands on March 1 in non-leap years.
...
- **Breadth:** four `new Date(y, m, d)` overflow sites in one function —
  `src/model/expansion.ts:225` (`bymonthday`), `:239` (monthly fallback on
  `anchor.getDate()`), `:242` (yearly), and the cursor at `:198` (`nextBase`). Affects
  every monthly series whose day-of-month exceeds 28 and every yearly Feb-29 series.
- **Recommended model:** **Sonnet 5.** The hazard: fixing `matchesInPeriod` alone leaves
  the cursor drifting — the skipped February comes from `nextBase`, not from the day
  computation — so a fix that clamps the emitted day still loses whole months, and the
  repro half-passes (no more May 1, still no February). Both must change together, and the
  cursor has to advance on a normalized month index rather than on the anchor's
  day-of-month.
- **Evidence:** `src/model/expansion.ts:225`:
  ...
  and `src/model/expansion.ts:198`:

  ```ts
      else if (freq === 'monthly') n.setMonth(n.getMonth() + interval)
  ```

- **Problem:** month-end arithmetic overflows into the next month, so a "31st" rule fires
  on the 1st and skips months entirely — the user's recurring reminder appears on the
  wrong day and is missing on others.
- **Fix:** clamp the generated day to the target month's length (or skip that month, per
  the project's chosen policy) and advance the cursor by normalized month index rather
  than by mutating a date carrying a day-of-month the next month may not have. After the
  fix the repro must contain only `-31` dates and must not contain `2026-05-01`.
