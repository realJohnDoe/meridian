...
- **Build a realistic test vault first.** The example vault is too small to expose scaling problems. Generate a vault at plausible heavy-use scale (on the order of a few hundred files / a couple thousand occurrences, including repeating entries) and run all measurements against it. Keep the generator script so measurements are reproducible; note the vault's size in the coverage statement.
