...
Rank findings by a rough `(impact × breadth) ÷ effort` intuition, where `effort` is the recommended-model tier read as an ordinal — Haiku 4.5 = 1, Sonnet 5 = 2, Opus 5 = 3, Opus 5 plan-mode/multi-PR = 5 — but report Impact, Breadth, and Recommended model as the separate fields above so the reader can re-sort by what they care about. Also add a short **summary table** (finding → recommended model) above the findings, so the tiers can be read at a glance without scrolling the full entries. Where findings touch the same code, add a one-line **sequencing note** saying which order avoids rebasing the same file twice.
