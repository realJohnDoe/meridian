...
- **Calendar subscriptions, import and export** — subscribe to an iCal feed (Google, Outlook, Apple, or anywhere else) and see its events alongside your own, read-only; or *import* an `.ics` export into a vault, which turns each event into an ordinary Markdown entry you own and can edit, so moving in doesn't mean retyping your calendar. A subscription you already have can be moved in the same way, so you can watch a calendar first and keep it second. A vault exports back out as a single `.ics` file to plug into another calendar.
